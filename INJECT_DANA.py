...
import os, sys, re, json, time, queue, threading, subprocess, asyncio, logging
...
# CONFIG
# ======================================================================
APP_VERSION = "3.0"  # Current app version for update check
...
DEFAULT_CONFIG = {
    ...
    "phone": "",
    "session_string": "",
    "group_chat_id": 0,       # Chat ID grup yang di-monitor
...
def adb_shell(device_id: str, *args) -> str:
    try:
        ...
        return subprocess.check_output(cmd, timeout=10, stderr=subprocess.STDOUT).decode("utf-8", errors="ignore")
    except Exception as e:
        return f"ERROR: {e}"
    ...
    def _reinit_uiautomator2(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "uiautomator2", "init", "--serial", self.device_id],
                    ...
                    try:
                        ...
                        for attempt in range(max_attempts):
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    err_msg = str(recreate_err).lower()
                                    # Check if it's a disconnection error
                                    if "disconnected" in err_msg or "connection" in err_msg or "eof" in err_msg:
                                        ...
                                        try:
                                            ...
                                            # Create fresh client with EMPTY session for QR login
                                            ...
                                            # Restart QR login with fresh session
                                        ...
                                        # Other error - might be QR scanned or auth related
                        ...
                        if "Two-steps verification" in err_str or "password is required" in err_str:
                            ...
                            if "LoginTokenMigrateTo" in err_str or "MigrateTo" in err_str:
                                self._log("QR scanned! Handling DC migration + 2FA...")
                                self.ui_q.put(("tg_qr_close", None))
                                
                                # Reconnect to handle DC migration
                                try:
                                    await self.client.disconnect()
                                    await asyncio.sleep(1)
                                    await self.client.connect()
                                    self._log("Reconnected after DC migration")
                                    
                                    # Now check if 2FA is needed
                                    try:
                                        if await self.client.is_user_authorized():
                                            logged_in_via_qr = True
                                            self._log("Already authorized after reconnect!")
                                        else:
                                    ...
                                    self._log(f"Migration/2FA handling failed: {e2fa}")
                            else:
...
class InjectDanaApp(QMainWindow):
    def __init__(self):
        ...
        # Set window icon
        _icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "lelepay_logo_real.ico")
        if not os.path.isfile(_icon_path):
            _icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "lelepay_logo.ico")
    ...
    def start_all(self):
        ...
        try:
            out = subprocess.check_output(["adb", "devices"], timeout=10, stderr=subprocess.STDOUT).decode("utf-8", errors="ignore")
    ...
    def _scan_for_new_devices(self):
        ...
        try:
            out = subprocess.check_output(
                ["adb", "devices"], timeout=10, stderr=subprocess.STDOUT
